document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.guide-card').forEach((card, index) => {
    const avatar = card.querySelector('.guide-avatar');
    const videoContainer = card.querySelector('.guide-video-container');
    const iframe = card.querySelector('iframe.guide-video');
  ...
  /* ---------------- Guide video (HTML5 video) ---------------- */
  document.querySelectorAll('.guide-avatar').forEach(avatar => {
    avatar.addEventListener('click', (e) => {
      e.preventDefault();
      const container = avatar.closest('.guide-card');
      const videoContainer = container.querySelector('.guide-video-container');
  ...
  document.querySelectorAll('.video-close-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.preventDefault();
      const videoContainer = btn.closest('.guide-video-container');
      const video = videoContainer.querySelector('video');
      video.pause();
